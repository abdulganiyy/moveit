...
const RequestDetails = ({ request, users }) => {
  ...
  const [dropper, setDropper] = useState<any>("");

  const [size, setSize] = useState<number>();

  const updateRequest = async (status: string) => {
    if (status === "intransit" && !dropper) {
      return alert(`Select a personel to drop off request`);
    }

    let dropperInfo = dropper.split(" ");

    try {
      setLoading(true);
      await Axios.patch(`/api/requests/${router.query.requestId}?role=picker`, {
        status,
        fullName: `${session.user.firstName} ${session.user.lastName}`,
        company: session.user?.company,
        comment,
        userId: session.user.uid,
        personelId: dropperInfo[0],
        personelFullName: `${dropperInfo[1]} ${dropperInfo[2]}`,
        email: dropperInfo[3],
      });
    } catch (error) {
    } finally {
    ...
    <DashboardLayoutWrapper>
      ...
      >
        <div className="min-h-[586px]">
          ...
          {request.dropper?.fullName && request.status === "completed" && (
            <div className="mt-4 bg-white py-[31px] px-[74px] rounded-[5px]">
              <h4>Delivered</h4>
              <div className="flex gap-x-[130px]">
                <Detail
          ...
          {request.status === "assigned" && (
            <div className="mt-4 flex justify-end gap-x-4">
              <select
                ...
                value={dropper}
                className="px-2 rounded-md outline-0"
              >
                <option value="">Assign a personel to drop off request</option>
                {users.map((user, i) => (
                  <option
                    key={i}
                    value={`${
...
export async function getServerSideProps(context) {
  ...
  const res = await Axios.get(
    `http${process.env.NODE_ENV === "production" ? "s" : ""}://${
      context.req.headers.host
    }/api/users?company=${session.user.company}&role=dropoff-personel`
  );
  const users = res.data.data;

  return {
    props: { session, request, users },
  };
}
