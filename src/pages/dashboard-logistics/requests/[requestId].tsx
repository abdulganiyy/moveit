...
const RequestDetails = ({ request, users }) => {
  ...
  const updateRequest = async (status: string) => {
    if (status === "assigned" && !picker) {
      return alert(`Select a personel to assign request`);
    }

    let pickerInfo = picker.split(" ");

    try {
      setLoading(true);
      await Axios.patch(
        `/api/requests/${router.query.requestId}?role=logistics`,
        {
          status,
          ...
          personelId: pickerInfo[0],
          personelFullName: `${pickerInfo[1]} ${pickerInfo[2]}`,
          email: pickerInfo[3],
        }
      );
    } catch (error) {
        ...
        <div className="min-h-[586px]">
          ...
          {request.status === "approved" && (
            <div className="mt-4 flex justify-end gap-x-4">
              ...
              >
                <option value="">Assign a personel</option>
                {users.map((user, i) => (
                  <option
                    key={i}
                    value={`${
              ...
              <Button
                onClick={() => updateRequest("assigned")}
                isLoading={loading}
                disabled={loading}
...
export async function getServerSideProps(context) {
  ...
  const res = await Axios.get(
    `http${process.env.NODE_ENV === "production" ? "s" : ""}://${
      context.req.headers.host
    }/api/users?company=${session.user.company}&role=pickup-personel`
  );
