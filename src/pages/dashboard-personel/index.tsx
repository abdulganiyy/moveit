...
const DashboardRequest = ({ requests }) => {
  ...
  const personelRequests = requests.filter(
    (request) => request.personel?.userId === session.user.uid
  );

  return (
    <DashboardLayout
      homelink="/dashboard-personel"
      links={[
        { path: "/dashboard-personel", name: "Dashboard" },
        { path: "/dashboard-personel/transactions", name: "Transactions" },
      ]}
    >
      ...
      <div className="bg-white p-2 py-[20px] rounded-[5px]">
        ...
        <Table items={personelRequests} url="personel" />
        <div className="flex justify-end py-5 px-5">
          <Link href="/dashboard-personel/transactions" className="underline">
            View All
          </Link>
        </div>
      </div>
